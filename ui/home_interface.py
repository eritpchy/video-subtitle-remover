...
class HomeInterface(QWidget):
    ...
    def on_task_error(self, e):
        self.append_output(f"任务执行出错: {str(e)}")
        print(self.current_processing_task_index)
        if self.current_processing_task_index >= 0:
            self.task_list_component.update_task_status(self.current_processing_task_index, TaskStatus.FAILED)
        ...
        if files:
            ...
            for path in reversed(files_loaded):
                ...
                if is_image_file(path):
                    output_path = os.path.abspath(os.path.join(os.path.dirname(self.video_path), f'{Path(self.video_path).stem}_no_sub.png'))
                else:
                    output_path = os.path.abspath(os.path.join(os.path.dirname(self.video_path), f'{Path(self.video_path).stem}_no_sub.mp4'))
